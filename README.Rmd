...
The package works only on MacOs systems. I don't expect anyone else than me to use this, but since I have done it, here it is...
...
The function generates the folder structure generally used for projects in our lab, and relative `README` files

You can run the function multiple types and you will be prompt to decide if overwrite or not particular folders.
...
Under the hood, the package `{labor}` use `rsync` to sync folders Set up the destination folder using `set_sync_lab`

![](inst/gifs_readme/set_destination.gif)

The destination folder is saved in a file file in the project directory `.labor_destination`.
...
3.  You can bidirectional sync the folders setting the argument `direction`
4.  You can still decide to use any `rsync` flags setting the argument `rsync_flags` (default is `-avtuP`).
